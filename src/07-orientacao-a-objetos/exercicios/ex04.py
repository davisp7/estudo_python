...
class Projeto:
    ...
    def add_participacao(self, participacao):
        if not isinstance(participacao, Participacao):
            raise TypeError("O objeto deve ser uma instância de Participacao")

        if participacao.projeto != self:
            raise ValueError("Esta participação pertence a outro projeto.")
